...
After publishing the custom rules to an OCI registry, you can configure a separate pipeline to use these rules. One way to do this is by using the [Update the Infrastructure as Code settings for a group API](https://apidocs.snyk.io/patch-/groups/-group_id-/settings/#patch-/groups/-group_id-/settings/iac).
